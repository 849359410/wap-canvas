...
const k3 = `${CDN_HOST}/img/lottery/eg_y/4.png`;
const pk10 = `${CDN_HOST}/img/lottery/eg_y/3.png`;
const ssc = `${CDN_HOST}/img/lottery/eg_y/1.png`;
...
let plan = {
	...
	getLottery(plan, that) {
		let game = JSON.parse(that.$stores.get("planGame"));
		plan.forEach(i => {
			game.forEach(j => {
	...
	planData(self) {
		self.$api.request("API_GET_WEB_IS_PLAN_STATE").then(
			success => {
				if (success.Data.is_plan !== 1) self.display = true;
			},
			err => this.$root.$emit("handleError", err)
		);
	}
};
export default plan;
